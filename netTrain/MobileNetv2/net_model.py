...
def correct_pad(inputs, kernel_size):
    ...
    # Returns
        A tuple.
    """
    img_dim = 1  # channel last in tensorflow
...
def MobileNetV2(input_shape=None,
    ...
    channel_axis = -1 # channel last in tensorflow

    first_block_filters = _make_divisible(32 * alpha, 8)
    x = keras.layers.ZeroPadding2D(padding=correct_pad(img_input, 3),
                                   name='Conv1_pad')(img_input)
    x = keras.layers.Conv2D(first_block_filters,
                            kernel_size=3,
                            strides=(2, 2),
                            padding='valid',
                            use_bias=False,
    ...
    if include_top:
        ...
        x = keras.layers.Dense(classes, activation='linear',
                               use_bias=True, name='traj')(x)
        x = keras.layers.LeakyReLU(alpha=0.1)(x)
    else:
        if pooling == 'avg':
            x = keras.layers.GlobalAveragePooling2D()(x)
    ...
    # Create model.
    model = keras.Model(inputs, x, name='mobilenetv2')

    # Load weights.
    if weights == 'imagenet':
        if include_top:
            model_name = ('mobilenet_v2_weights_tf_dim_ordering_tf_kernels_' +
                          str(alpha) + '_' + str(224) + '.h5')
