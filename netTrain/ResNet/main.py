# Modify based on "tensorflow model" Resnet project
# https://github.com/tensorflow/models/tree/r1.13.0/official/resnet
# ==============================================================================
"""Runs a ResNet model on the carla stimulated dataset."""
...
def parse_record(raw_record, dtype):
    ...
    image = tf.io.decode_png(image_buffer, channels=3, dtype=tf.dtypes.uint8)
    image.set_shape([IMAGE_HEIGHT, IMAGE_WIDTH, NUM_CHANNELS])
    image = tf.cast(image, dtype)

    traj = tf.decode_raw(traj_buffer, tf.dtypes.int16)
    traj.set_shape([NUM_TIME_SEQUENCE, 2])
...
class ImagenetModel(resnet_model.Model):
  ...
  def __init__(self, resnet_size, data_format=None, num_classes=NUM_TIME_SEQUENCE,
...
def imagenet_model_fn(features, trajs, mode, params):
