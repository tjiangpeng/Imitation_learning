...
DATA_LOG_DIR = "../../data/2019_08_14/validation/"
IMG_HEIGHT = 384
IMG_WIDTH = 384
##################################################################################
...
class World(object):
    ...
    def start(self, hud, input_control):
        ...
        self.original_surface_size = min(self._hud.dim[0], self._hud.dim[1]) * 1.5
        ...
        left_top_offset = (int(self._hud.dim[0] / 2) - int(self._hud.dim[1] / 2), 0)
        rect = pygame.Rect(left_top_offset, (self._hud.dim[1], self._hud.dim[1]))
        pygame.draw.rect(self.border_round_surface, COLOR_ALUMINIUM_1, rect)
        rect = pygame.Rect((int(self._hud.dim[0] / 2) - int(self._hud.dim[1] / 2) + 8, 8), (self._hud.dim[1]-16, self._hud.dim[1]-16))
        pygame.draw.rect(self.border_round_surface, COLOR_WHITE, rect)
        ...
        for v in list_v:
            ...
            if v[0].attributes['role_name'] == 'hero':
                ...
                if len(self.hero_past_traj) == PAST_TRAJECTORY_TIME_INTERVAL:
                    # decrease = int(255/(PAST_TRAJECTORY_TIME_INTERVAL+1)) - 1
                    # intensity = 255 - decrease
                    # for past_pos in self.hero_past_traj:
                    #     pos = world_to_pixel(carla.Location(x=past_pos[0], y=past_pos[1]))
                    #     color = pygame.Color(255, intensity, intensity)
                    #     intensity = intensity - decrease
                ...
                if v[0].id in self.actors_past_traj:
                    if len(self.actors_past_traj[v[0].id]) == PAST_TRAJECTORY_TIME_INTERVAL:
                        # decrease = int(255 / (PAST_TRAJECTORY_TIME_INTERVAL + 1)) - 1
                        # intensity = 255 - decrease
                        # for past_pos in self.actors_past_traj[v[0].id]:
                        #     pos = world_to_pixel(carla.Location(x=past_pos[0], y=past_pos[1]))
                        #     color = pygame.Color(intensity, intensity, 255)
                        #     intensity = intensity - decrease
                        #     pygame.draw.circle(surface, color, pos, 2)
    ...
    def write_to_txt(self, ind, list_v):
        waypoint = self.town_map.get_waypoint(self.hero_actor.get_location())
            ...
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    # log dataset
                        ...
                        self._hud.notification('Log data %s' % ('On' if self._log_enabled else 'Off'))
